...
void inverse_matrix()
{
	...
	{
		cout << endl << "Обратную матрицу невозможно найти, так как детерминант матрицы равен нулю." << endl << endl;
		return;
	}
...
void determinant()
{
	...
	do {
		...
		cin >> matrix.size;
		if (matrix.size < 1) {
			cout << "Размер матрицы не может быть меньше 1.\n";
		}
	} while (matrix.size < 1);
